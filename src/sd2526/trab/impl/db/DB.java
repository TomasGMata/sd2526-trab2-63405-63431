...
public class DB {
    ...
    public static <T> Result<T> deleteOne(T obj) {
        return reTry(() -> Hibernate.getInstance().transaction(hibernate -> hibernate.deleteOne(obj)));
    }

    public static <T> Result<T> updateOne(T obj) {
        return reTry(() -> Hibernate.getInstance().transaction(hibernate -> hibernate.updateOne(obj)));
    }

    public static <T> Result<T> persistOne(T obj) {
        return reTry(() -> Hibernate.getInstance().transaction(hibernate -> hibernate.persistOne(obj)));
    }
